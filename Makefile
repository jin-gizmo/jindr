...
.PHONY: black help _venv_is_off _venv_is_on _venv update check doc test
...
help:
	...
	@echo "   pypi:      Upload the pkg to the \"$(pypi)\" PyPI server via twine."
	@echo "              The \"$(pypi)\" server must be defined in ~/.pypirc."
	@echo "              Add pypi=... to specify a different index server entry in ~/.pypirc."
	@echo
